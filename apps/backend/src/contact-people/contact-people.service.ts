...
export class ContactPeopleService {
      ...
      if (error instanceof Prisma.PrismaClientKnownRequestError) {
        if (error.code === 'P2002') {
          ...
          throw new ConflictException(
            `A contact person with this ${target.join(', ')} already exists.`
          )
        }
      }
      throw error
  ...
  async findOne(
    ...
    const contactPerson = await this.prisma.contactPerson.findFirst({
      where: { id, tenantId },
    })
    if (!contactPerson || contactPerson.companyId !== companyId) {
      throw new NotFoundException(
        `ContactPerson with ID ${id} not found in company ${companyId}.`
      )
    }
