...
export class QuotesController {
  ...
  async create(@TenantId() tenantId: string, @Body() createQuoteDto: CreateQuoteDto, @Request() req) {
    try {
      return await this.quotesService.create(tenantId, createQuoteDto, req.user.id);
    } catch (error) {
  ...
  findAll(
    ...
    return this.quotesService.findAll(tenantId, page, limit);
  }
  ...
  @Patch(':id')
  @Roles(UserRole.ADMIN, UserRole.EDITOR)
  async update(@TenantId() tenantId: string, @Param('id') id: string, @Body() updateQuoteDto: UpdateQuoteDto, @Request() req) {
    try {
      return await this.quotesService.update(tenantId, id, updateQuoteDto, req.user.id);
    } catch (error) {
      if (error instanceof NotFoundException || error instanceof BadRequestException) {
        throw error;
      }
