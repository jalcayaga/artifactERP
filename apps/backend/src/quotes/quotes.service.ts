...
export class QuotesService {
  ...
  async create(tenantId: string, createQuoteDto: CreateQuoteDto, userId: string): Promise<Quote> {
    const { items, companyId, ...quoteData } = createQuoteDto;

    return this.prisma.$transaction(async (tx) => {
      await this.ensureUserBelongsToTenant(tenantId, userId);
      await this.ensureCompanyBelongsToTenant(tenantId, companyId, userId);
      ...
      for (const itemDto of items) {
        const product = await this.prisma.product.findFirst({ where: { id: itemDto.productId, tenantId } });
        if (!product) {
  ...
  async findAll(tenantId: string, page: number, limit: number): Promise<{ data: Quote[]; total: number; pages: number; currentPage: number }> {
  ...
  async update(tenantId: string, id: string, updateQuoteDto: UpdateQuoteDto, userId: string): Promise<Quote> {
  ...
  private async ensureCompanyBelongsToTenant(tenantId: string, companyId: string, userId: string) {
  ...
  private async ensureQuoteBelongsToTenant(tenantId: string, quoteId: string) {
    const quote = await this.prisma.quote.findFirst({ where: { id: quoteId, tenantId } });
    if (!quote) {
      throw new ForbiddenException(`Quote ${quoteId} does not belong to tenant ${tenantId}`);
    }
  }
}
