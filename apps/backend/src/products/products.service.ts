...
export class ProductsService {
  ...
  async findPublished(
    ...
    page: number,
    limit: number,
    filters: { category?: string; searchQuery?: string; minPrice?: number; maxPrice?: number } = {},
  ) {
  ...
  async getAverageCost(tenantId: string, id: string): Promise<{ averageCost: number }> {
  ...
  async updateTechnicalSheet(tenantId: string, id: string, filename: string): Promise<Product> {
  ...
  async getLotsByProductId(tenantId: string, productId: string): Promise<Lot[]> {
  ...
  private async ensureBelongsToTenant(tenantId: string, productId: string): Promise<Product> {
