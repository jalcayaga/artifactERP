...
export class PurchasesService {
  ...
  async update(tenantId: string, id: string, dto: UpdatePurchaseDto): Promise<Purchase> {
    ...
    return this.prisma.purchase.update({ where: { id }, data, include: { company: true, items: true } });
  }
  ...
  private async ensurePurchaseBelongsToTenant(tenantId: string, purchaseId: string) {
    const purchase = await this.prisma.purchase.findFirst({ where: { id: purchaseId, tenantId } });
    if (!purchase) {
      throw new ForbiddenException(`Purchase ${purchaseId} does not belong to tenant ${tenantId}`);
    }
  }
}
