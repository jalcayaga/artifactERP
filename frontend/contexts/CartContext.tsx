...
export const CartProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const addItem = (product: Product, quantity: number, includeInstallation: boolean = false) => {
    setItems(prevItems => {
      ...
      if (existingItemIndex > -1) {
        const updatedItem = {
          ...
          installationService: installationServiceDetails !== undefined ? installationServiceDetails : newItems[existingItemIndex].installationService,
        };
  ...
  return (
    <CartContext.Provider value={{ items, addItem, removeItem, updateItemQuantity, clearCart, cartTotal, itemCount }}>
      {children}
