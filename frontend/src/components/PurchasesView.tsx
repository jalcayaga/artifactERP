...
import { PlusIcon, TruckIcon, EyeIcon } from '@/components/Icons';
                    ...
                    {purchaseOrders.map((order) => (
                      <tr key={order.id} className="hover:bg-accent transition-colors duration-150">
                        ...
                        <td className="px-4 py-3 whitespace-nowrap">
                          ...
                          <div className="text-xs text-muted-foreground sm:hidden">{new Date(order.orderDate).toLocaleDateString()} - Total: ${order.grandTotal.toFixed(2)}</div>
                        ...
                        <td className="px-4 py-3 whitespace-nowrap text-sm text-muted-foreground text-right hidden md:table-cell">${order.subTotal.toFixed(2)}</td>
                        <td className="px-4 py-3 whitespace-nowrap text-sm text-muted-foreground text-right hidden lg:table-cell">${order.totalVatAmount.toFixed(2)}</td>
                        <td className="px-4 py-3 whitespace-nowrap text-sm font-semibold text-primary text-right">${order.grandTotal.toFixed(2)}</td>
