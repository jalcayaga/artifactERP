...
const StatCard: React.FC<StatCardProps> = ({ title, value, icon: Icon }) => {
  return (
    <Card className="overflow-hidden">
      <CardContent className="p-4"> {/* shadcn cards suelen tener padding en CardContent, p-4 es un buen default */}
        <div className="flex items-center">
          <div className={`p-3 rounded-lg bg-primary/10 dark:bg-primary/20 mr-4`}> {/* Updated icon background */}
            <Icon className={`w-6 h-6 text-primary`} /> {/* Updated icon color */}
          </div>
          <div>
        ...
        <Card>
          ...
          <CardContent className="pt-2">
            <ul className="space-y-3">
              {[
                { id: 1, text: "Revisar propuesta Cliente Alfa", due: "Mañana" },
                { id: 2, text: "Contactar Proveedor Beta", due: "En 3 días" },
                { id: 3, text: "Generar reporte de ventas Q3", due: "Próxima semana" },
                { id: 4, text: "Actualizar inventario", due: "Hoy" },
              ].map(task => (
                <li key={task.id} className="flex items-center justify-between">
                  <div>
                    <p className="text-sm font-medium text-foreground">{task.text}</p>
                    <p className="text-xs text-muted-foreground">Vence: {task.due}</p>
                  </div>
