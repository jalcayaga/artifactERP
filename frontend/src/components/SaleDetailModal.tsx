...
import { Sale, SaleItem } from "@/types"; // Tipos de datos para la venta y sus artículos.
import {
  XIcon,
  ShoppingCartIcon,
  CalendarIcon,
  ChatBubbleLeftEllipsisIcon,
} from "@/icons"; // Iconos para la UI del modal.
                ...
                {sale.items.map((item) => (
                  <tr key={item.id}>
                    ...
                    <td className="px-3 py-2.5 whitespace-nowrap text-sm font-medium text-foreground text-right">
                      ${calculateItemTotalWithVat(item).toFixed(2)}
                    </td>
                  </tr>
                ))}
              </tbody>
