...
import { Client } from "@/types"; // Tipo de dato para un cliente.
import { XIcon, BriefcaseIcon, UserCircleIcon as PersonIcon } from "@/icons"; // Iconos para la UI del modal.
  ...
  return (
    <div className="flex flex-col sm:flex-row sm:items-start py-2">
      ...
      <dd className="w-full sm:w-2/3 mt-1 sm:mt-0 text-sm text-foreground">
        ...
        {/* Muestra "N/A" si el valor es falsy pero no undefined/empty string manejado arriba. */}
      </dd>
    </div>
  );
};
      ...
      >
          ...
          >
            {/* Icono según el tipo de cliente */}
            {
              client.type === "Empresa" ? (
                ...
                <PersonIcon className="w-6 h-6 mr-2 text-green-500" />
              ) // Keeping green for Persona type, adjust if needed
            }
        ...
        <div className="p-4 sm:p-5 space-y-1 overflow-y-auto">
          ...
          <dl className="divide-y divide-border/50">
            {" "}
            {/* Lista de definiciones para los detalles. */}
            {client.contactName && (
              <DetailItem
            ...
            <DetailItem
              label="Tipo"
              value={
                // Muestra el tipo de cliente con una etiqueta coloreada.
                <span
