...
const QuoteDetailModal: React.FC<QuoteDetailModalProps> = ({ quote, onClose }) => {
  return (
    <Dialog open={true} onOpenChange={onClose}>
      <DialogContent className="max-w-3xl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 py-4">
          ...
          {quote.notes && <div className="col-span-2"><strong>Notas:</strong> {quote.notes}</div>}
        </div>
          ...
          <TableBody>
            {quote.items?.map(item => (
              <TableRow key={item.id}>
                ...
                <TableCell className="text-right">{formatCurrencyChilean(parseFloat(item.unitPrice as any))}</TableCell>
                <TableCell className="text-right">{formatCurrencyChilean(parseFloat(item.totalPriceWithVat as any))}</TableCell>
              </TableRow>
            ))}
          </TableBody>
        </Table>
        <div className="flex justify-end mt-4 space-x-4 font-semibold">
          <div>Subtotal:</div>
          <div>{formatCurrencyChilean(parseFloat(quote.subTotalAmount as any))}</div>
        </div>
        <div className="flex justify-end mt-2 space-x-4 font-semibold">
          <div>IVA (19%):</div>
          <div>{formatCurrencyChilean(parseFloat(quote.totalVatAmount as any))}</div>
        </div>
        <div className="flex justify-end mt-2 space-x-4 font-bold text-lg">
          <div>Total:</div>
          <div>{formatCurrencyChilean(parseFloat(quote.grandTotalAmount as any))}</div>
        </div>
        <DialogFooter>
