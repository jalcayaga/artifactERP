...
const QuoteForm: React.FC<QuoteFormProps> = ({ quoteData, onSave, onCancel }) => {
  ...
  const calculateTotals = useCallback(() => {
    ...
    items.forEach((item, index) => {
      ...
      if (product) {
        const unitPrice = parseFloat(form.getValues(`items.${index}.unitPrice`) as any || product.price as any);
        ...
        form.setValue(`items.${index}.itemVatAmount`, vatAmount, { shouldValidate: true });
        form.setValue(`items.${index}.totalPriceWithVat`, totalPriceWithVat, { shouldValidate: true });
  ...
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
        <Card>
          <CardHeader>
            <CardTitle>{quoteData ? 'Editar Cotización' : 'Crear Cotización'}</CardTitle>
          </CardHeader>
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Textarea placeholder="Notas adicionales sobre la cotización" {...field} />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}
        ...
        <Card>
          ...
          <CardContent>
            {fields.map((field, index) => (
              <div key={field.id} className="grid grid-cols-1 md:grid-cols-5 gap-4 mb-4 items-center">
                <FormField
                  control={form.control}
                  ...
                  render={({ field }) => (
                    <FormItem className="col-span-2">
                      <Select onValueChange={(value) => {
                        ...
                        form.setValue(`items.${index}.unitPrice`, parseFloat(product?.price as any || '0'));
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <Input type="number" placeholder="Cantidad" {...field} onChange={e => field.onChange(parseInt(e.target.value, 10) || 0)} />
                    </FormItem>
                  )}
                />
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <Input type="text" placeholder="Precio Unit." {...field} onChange={e => field.onChange(parseChileanCurrency(e.target.value))} value={formatCurrencyChilean(field.value)} />
                    </FormItem>
                  )}
                />
