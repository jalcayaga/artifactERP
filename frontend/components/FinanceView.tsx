...
const FinanceStatCard: React.FC<StatCardProps> = ({
  title,
  value,
  icon: Icon,
      ...
      options: {
        ...
        plugins: {
          ...
          tooltip: {
            ...
            callbacks: {
              label: (context) =>
                `${context.dataset.label}: ${formatCurrencyChilean(context.parsed.y)}`,
            },
          },
        },
...
const FinanceView: React.FC = () => {
  const mockAccountsReceivable = [
    ...
    {
      id: 'ar2',
      client: 'Servicios Beta Ltda.',
      invoice: 'F00115',
      amount: 875000,
      dueDate: 'VENCIDA (Hace 10 días)',
      overdue: true,
    },
    {
      id: 'ar3',
      client: 'Comercial Gamma',
      invoice: 'F00128',
      amount: 2300000,
      dueDate: 'Vence en 15 días',
      overdue: false,
    },
  ]
  ...
  const mockAccountsPayable = [
    ...
    {
      id: 'ap3',
      supplier: 'Logística Delta',
      invoice: 'LD-321',
      amount: 115000,
      dueDate: 'Vence en 20 días',
      urgent: false,
    },
  ]
  ...
  const mockRecentTransactions = [
    {
      id: 'tx1',
      type: 'Ingreso',
      description: 'Pago Factura F00110 - Cliente Alfa',
      date: 'Hoy',
      amount: 1850000,
    },
    {
      id: 'tx2',
      type: 'Egreso',
      description: 'Pago Proveedor Zeta Inc. PZ-980',
      date: 'Ayer',
      amount: 650000,
    },
    {
      id: 'tx3',
      type: 'Ingreso',
      description: 'Abono Proyecto Beta',
      date: 'Hace 2 días',
      amount: 950000,
    },
    {
      id: 'tx4',
      type: 'Egreso',
      description: 'Compra de Insumos Oficina',
      date: 'Hace 3 días',
      amount: 85000,
    },
    {
      id: 'tx5',
      type: 'Egreso',
      description: 'Pago arriendo oficina Noviembre',
      date: 'Hace 5 días',
      amount: 1200000,
    },
  ]
  ...
  return (
    <div className='space-y-6 lg:space-y-8'>
      <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6'>
        <FinanceStatCard
          title='Saldo Actual (Cuentas)'
          value={formatCurrencyChilean(15750000)}
          icon={BanknotesIcon}
        />
        <FinanceStatCard
          title='Ingresos del Mes'
          value={formatCurrencyChilean(7850000)}
          icon={TrendingUpIcon}
        ...
        <FinanceStatCard
          ...
          value={formatCurrencyChilean(4320000)}
        ...
        <FinanceStatCard
          ...
          value={`${formatCurrencyChilean(750000)} (Hoy)`}
      ...
      <div className='grid grid-cols-1 lg:grid-cols-2 gap-6'>
        <Card>
          <CardHeader>
            <CardTitle className='text-lg'>Estado de Cuentas</CardTitle>
          </CardHeader>
          <CardContent className='space-y-6'>
            <div>
              ...
              <ul className='space-y-2.5 text-sm'>
                {mockAccountsReceivable.map((item) => (
                  <li
                    key={item.id}
                    className={`p-2.5 rounded-md ${item.overdue ? 'bg-amber-500/10' : 'bg-muted/50'}`}
                  >
                    <div className='flex justify-between items-center'>
                      ...
                      <span
                        className={`font-semibold ${item.overdue ? 'text-amber-600 dark:text-amber-400' : 'text-foreground'}`}
                      >
                        {formatCurrencyChilean(item.amount)}
                      </span>
                    </div>
                    <p
                      className={`text-xs ${item.overdue ? 'text-amber-500' : 'text-muted-foreground'}`}
                    >
                      {item.dueDate}
                    </p>
                  </li>
            ...
            <div>
              <h3 className='text-md font-semibold text-rose-600 dark:text-rose-400 mb-2'>
                Cuentas por Pagar
              </h3>
              <ul className='space-y-2.5 text-sm'>
                {mockAccountsPayable.map((item) => (
                  <li
                    key={item.id}
                    className={`p-2.5 rounded-md ${item.urgent ? 'bg-rose-500/10' : 'bg-muted/50'}`}
                  >
                    <div className='flex justify-between items-center'>
                      <span className='font-medium text-foreground'>
                        {item.supplier}{' '}
                      ...
                      <span
                        className={`font-semibold ${item.urgent ? 'text-rose-600 dark:text-rose-400' : 'text-foreground'}`}
                      >
                        {formatCurrencyChilean(item.amount)}
                      </span>
                    </div>
                    <p
                      className={`text-xs ${item.urgent ? 'text-rose-500' : 'text-muted-foreground'}`}
                    >
                      {item.dueDate}
                    </p>
                  </li>
            ...
            <ul className='space-y-3'>
              {mockRecentTransactions.map((tx) => (
                ...
                >
                  ...
                  <span
                    className={`text-sm font-semibold ${tx.type === 'Ingreso' ? 'text-emerald-600 dark:text-emerald-400' : 'text-rose-600 dark:text-rose-400'}`}
                  >
                    {tx.type === 'Ingreso' ? '+' : '-'}
                    {formatCurrencyChilean(tx.amount)}
                  </span>
                </li>
              ))}
            </ul>
            <button className='mt-4 text-sm font-medium text-primary hover:text-primary/80 w-full text-center'>
