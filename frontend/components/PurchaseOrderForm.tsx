...
const PurchaseOrderForm: React.FC<PurchaseOrderFormProps> = ({ onSave, onCancel }) => {
  ...
  const validateMainForm = (): boolean => {
    ...
    if (items.length === 0) newErrors.items = 'Debe añadir al menos un artículo a la orden de compra.';
    setErrors(newErrors);
      ...
      <form onSubmit={handleSubmit}>
        <CardContent className="space-y-6 pt-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <Label htmlFor="company-select">Proveedor <span className="text-red-500">*</span></Label>
              ...
              {errors.selectedCompanyId && <p id="company-error" className={errorTextClass}>{errors.selectedCompanyId}</p>}
            </div>
            <div>
              <Label htmlFor="order-date">Fecha de Pedido <span className="text-red-500">*</span></Label>
              <Input type="date" id="order-date" value={orderDate} onChange={(e) => setOrderDate(e.target.value)} />
              {errors.orderDate && <p id="orderDate-error" className={errorTextClass}>{errors.orderDate}</p>}
            </div>
          </div>

          <div>
            <Label htmlFor="expected-delivery-date">Fecha de Entrega Esperada</Label>
            <Input type="date" id="expected-delivery-date" value={expectedDeliveryDate} onChange={(e) => setExpectedDeliveryDate(e.target.value)} />
          </div>

          <div className="space-y-4 p-4 border border-border rounded-md bg-card">
            <h3 className="text-md font-semibold text-foreground">Artículos de la Orden de Compra</h3>
            ...
            ) : ( <p className="text-sm text-center text-muted-foreground italic py-3">Aún no se han añadido artículos a la orden de compra.</p> )}
