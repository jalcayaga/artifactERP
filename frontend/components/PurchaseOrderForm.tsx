...
const PurchaseOrderForm: React.FC<PurchaseOrderFormProps> = ({ orderData, onSave, onCancel }) => {
  ...
  const validateMainForm = (): boolean => {
    ...
    if (!supplierName.trim()) newErrors.supplierName = 'El nombre del proveedor es requerido.';
    if (!orderDate) newErrors.orderDate = 'La fecha de la orden es requerida.';
    if (items.length === 0) newErrors.items = 'Debe añadir al menos un artículo a la orden.';
    setErrors(newErrors);
        ...
        <CardContent className="space-y-6 pt-6">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="md:col-span-1">
              <label htmlFor="po-supplier-name" className="block text-sm font-medium text-foreground">
                Proveedor <span className="text-red-500">*</span>
              </label>
              <input type="text" id="po-supplier-name" value={supplierName} onChange={(e) => setSupplierName(e.target.value)} className={inputBaseClass} placeholder="Nombre del proveedor" aria-describedby="supplierName-error"/>
              {errors.supplierName && <p id="supplierName-error" className={errorTextClass}>{errors.supplierName}</p>}
            </div>
            <div>
          ...
          <div className="space-y-4 p-4 border border-border rounded-md bg-card"> 
            ...
            <div className="grid grid-cols-1 sm:grid-cols-12 gap-4 items-end">
              <div className="sm:col-span-5">
                  <label htmlFor="po-item-name" className="block text-xs font-medium text-muted-foreground">Producto / Servicio</label>
                  <input type="text" id="po-item-name" value={newItemName} onChange={e => setNewItemName(e.target.value)} placeholder="Nombre del artículo" className={`${inputBaseClass} py-1.5 text-sm`} aria-describedby="poItemName-error" />
                  {itemErrors.name && <p id="poItemName-error" className={errorTextClass}>{itemErrors.name}</p>}
              </div>
              <div className="sm:col-span-2">
                  <label htmlFor="po-item-quantity" className="block text-xs font-medium text-muted-foreground">Cant.</label>
                  <input type="number" id="po-item-quantity" value={newItemQuantity} onChange={e => setNewItemQuantity(e.target.value)} placeholder="0" className={`${inputBaseClass} py-1.5 text-sm`} aria-describedby="poItemQuantity-error" />
                  {itemErrors.quantity && <p id="poItemQuantity-error" className={errorTextClass}>{itemErrors.quantity}</p>}
              </div>
              <div className="sm:col-span-3">
                  <label htmlFor="po-item-price" className="block text-xs font-medium text-muted-foreground">P. Unit. (s/IVA)</label>
                  <input type="number" id="po-item-price" value={newItemPrice} onChange={e => setNewItemPrice(e.target.value)} placeholder="0.00" step="0.01" className={`${inputBaseClass} py-1.5 text-sm`} aria-describedby="poItemPrice-error" />
                  {itemErrors.price && <p id="poItemPrice-error" className={errorTextClass}>{itemErrors.price}</p>}
              </div>
              <div className="sm:col-span-2">
                  <button type="button" onClick={handleAddItem} className="w-full bg-accent hover:bg-accent/80 text-accent-foreground font-medium py-2 px-3 rounded-md shadow-sm text-sm flex items-center justify-center space-x-1.5 transition-all" aria-label="Añadir artículo a la orden">
                ...
                <table className="min-w-full divide-y divide-border">
                  <thead className="bg-muted/50">
                    <tr>
                      ...
                      <th className="px-3 py-2 text-right text-xs font-semibold text-muted-foreground uppercase tracking-wider hidden sm:table-cell">IVA Artículo</th>
                      <th className="px-3 py-2 text-right text-xs font-semibold text-muted-foreground uppercase tracking-wider">Total Artículo</th>
            ...
            ) : ( <p className="text-sm text-center text-muted-foreground italic py-3">Aún no se han añadido artículos a la orden.</p> )}
