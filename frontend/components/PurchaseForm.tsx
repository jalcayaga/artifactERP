...
import { Product, Company, CreatePurchaseDto, PaginatedResponse } from '@/lib/types';
...
const PurchaseForm: React.FC<PurchaseFormProps> = ({ onSave, onCancel }) => {
      ...
      CompanyService.getAllCompanies()
        .then(res => {
          if (Array.isArray(res)) {
            setCompanies(res.filter(c => c.isSupplier));
          } else {
            setCompanies(res.data.filter(c => c.isSupplier));
          }
        })
  ...
  const validateItemForm = (): boolean => {
    ...
    if (!selectedProductToAdd) newItmErrors.product = 'Debe seleccionar un producto.';
  ...
  const validateMainForm = (): boolean => {
    ...
    if (!selectedCompanyId) newErrors.selectedCompanyId = 'Debe seleccionar un proveedor.';
    if (!purchaseDate) newErrors.purchaseDate = 'La fecha de compra es requerida.';
    if (items.length === 0) newErrors.items = 'Debe añadir al menos un artículo a la compra.';
    setErrors(newErrors);
      ...
      <form onSubmit={handleSubmit}>
        <CardContent className="space-y-6 pt-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <Label htmlFor="supplier-select">Proveedor <span className="text-red-500">*</span></Label>
              ...
              {errors.selectedCompanyId && <p id="supplier-error" className={errorTextClass}>{errors.selectedCompanyId}</p>}
            </div>
            <div>
              <Label htmlFor="purchase-date">Fecha de Compra <span className="text-red-500">*</span></Label>
              <Input type="date" id="purchase-date" value={purchaseDate} onChange={(e) => setPurchaseDate(e.target.value)} />
              {errors.purchaseDate && <p id="purchaseDate-error" className={errorTextClass}>{errors.purchaseDate}</p>}
            </div>
          </div>

          <div className="space-y-4 p-4 border border-border rounded-md bg-card">
            <h3 className="text-md font-semibold text-foreground">Artículos de la Compra</h3>
            <div className="grid grid-cols-1 sm:grid-cols-12 gap-4 items-end">
              <div className="sm:col-span-5 relative">
                ...
                {itemErrors.product && <p className={errorTextClass}>{itemErrors.product}</p>}
              </div>
                ...
                <div className="sm:col-span-7 grid grid-cols-1 sm:grid-cols-5 gap-4">
                  <div className="sm:col-span-2">
                    ...
                    <Input
                      ...
                      placeholder="0"
                    />
                    {itemErrors.quantity && <p className={errorTextClass}>{itemErrors.quantity}</p>}
                  </div>
                  <div className="sm:col-span-3">
                    ...
                    <Input
                      ...
                      step="0.01"
                    />
                    {itemErrors.price && <p className={errorTextClass}>{itemErrors.price}</p>}
                  </div>
              ...
              <div className="mt-4 -mx-4 sm:mx-0 overflow-x-auto">
                <table className="min-w-full divide-y divide-border">
                  <thead className="bg-muted/50">
                    <tr>
                      <th className="px-3 py-2 text-left text-xs font-semibold text-muted-foreground uppercase tracking-wider">Producto</th>
                      <th className="px-3 py-2 text-right text-xs font-semibold text-muted-foreground uppercase tracking-wider">Cant.</th>
                      <th className="px-3 py-2 text-right text-xs font-semibold text-muted-foreground uppercase tracking-wider">Costo Unit. (s/IVA)</th>
                  ...
                  <tbody className="divide-y divide-border">
                    {items.map((item, index) => (
                      <tr key={index}>
                        ...
                        <td className="px-3 py-2 whitespace-nowrap text-sm text-foreground font-medium text-right">{formatCurrencyChilean(item.totalPrice)}</td>
                        <td className="px-3 py-2 whitespace-nowrap text-center">
                          <button type="button" onClick={() => handleRemoveItem(index)} title="Eliminar Artículo" className="text-destructive hover:text-destructive/80 p-1 rounded-md hover:bg-destructive/10 transition-colors">
            ...
            ) : ( <p className="text-sm text-center text-muted-foreground italic py-3">Aún no se han añadido artículos a la compra.</p> )}
          ...
          <div className="mt-6 pt-4 border-t border-border space-y-2">
            ...
            <div className="flex justify-end items-center mt-1 pt-1 border-t border-border/50"><span className="text-lg font-bold text-foreground">Total Compra:</span><span className="ml-4 text-2xl font-bold text-primary w-32 text-right">{formatCurrencyChilean(grandTotal)}</span></div>
          </div>
        </CardContent>
