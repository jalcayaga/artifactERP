...
import chileanRegions from '@/lib/chilean_regions.json'
...
const CompanyForm: React.FC<CompanyFormProps> = ({
  ...
  useEffect(() => {
    ...
    if (companyData) {
      ...
      // setBillingPhone(companyData.phone || '') // Old phone field

      // Set selected country and phone number
      ...
      // Reset contact fields when editing existing company
      setContactFirstName('')
      setContactLastName('')
      setContactEmail('')
      setContactPhone('')
      ...
      setZip('')
      setEmail('')
      // setBillingPhone('') // Old phone field
      setSelectedCountry(defaultCountry)
      setPhoneNumber('')
      // Set isClient/isSupplier based on form type
      if (isMyCompanyForm) {
        setIsClient(false)
        setIsSupplier(false)
      } else {
      ...
      setContactEmail('')
      setContactPhone('')
      setContactRole('') // Ensure contactRole is always a string
    }
    setErrors({})
  ...
  const validateForm = (): boolean => {
    ...
    // Validate contact fields only for new company creation
    if (!companyData) {
    ...
    try {
      const companyDetails: CreateCompanyDto = {
        ...
        phone: selectedCountry && phoneNumber.trim()
          ...
          : undefined,
        isClient: isMyCompanyForm ? false : isClient, // Force false if it's my company form
        isSupplier: isMyCompanyForm ? false : isSupplier, // Force false if it's my company form
      }
      ...
      if (companyData) {
        ...
        // Add contact person details for new company creation
        savedCompany = await CompanyService.createCompany(companyDetails)

        // If contact person details are provided, create the contact person separately
