...
const ProductDetailView: React.FC<ProductDetailViewProps> = ({ productId }) => {
    ...
    const fetchProduct = async () => {
      try {
        ...
        const fetchedProduct =
          await ProductService.getPublishedProductById(productId)
        if (fetchedProduct) {
  ...
  if (isLoading) {
    return (
      <div className='container mx-auto px-4 py-12 text-center text-muted-foreground min-h-[60vh] flex items-center justify-center'>
  ...
  if (error) {
    return (
      <div className='container mx-auto px-4 py-12 text-center text-destructive min-h-[60vh] flex items-center justify-center'>
        {error}
      </div>
    )
  }
  ...
  const canAddToCart =
    isService || (stockAvailable && product.totalStock! >= quantity)
      ...
      <div className='grid grid-cols-1 lg:grid-cols-5 gap-8 lg:gap-12 items-start'>
        ...
        <div className='lg:col-span-3 space-y-6'>
          <div className='space-y-2'>
            ...
            {product.sku && (
              <p className='text-xs text-muted-foreground flex items-center'>
                <TagIcon className='w-3.5 h-3.5 mr-1.5' /> SKU: {product.sku}
              </p>
            )}
          </div>
          ...
          {product.technicalSheetUrl && (
            <div className='pt-4'>
              <a
                ...
                className='inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-secondary hover:bg-secondary/80 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-secondary'
              >
          ...
          {product.productType === 'PRODUCT' &&
            ...
            product.totalStock !== undefined && (
              <p
                className={`text-sm font-medium flex items-center ${product.totalStock > 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-destructive'}`}
              >
            ...
            <CardContent className='p-4 space-y-4'>
              ...
              <div className='flex items-center space-x-3'>
                ...
                <div className='flex items-center border border-input rounded-md bg-background shadow-sm'>
                  <button
                    ...
                    className='px-3 py-2 text-lg text-muted-foreground hover:bg-accent rounded-l-md transition-colors focus:outline-none focus:ring-1 focus:ring-primary'
                  ...
                  <button
                    onClick={() => handleQuantityChange(1)}
                    className='px-3 py-2 text-lg text-muted-foreground hover:bg-accent rounded-r-md transition-colors focus:outline-none focus:ring-1 focus:ring-primary'
              ...
              {product.productType === 'PRODUCT' && (
                <div className='flex items-center space-x-2.5 pt-2'>
                  ...
                  />
                  <label
                    htmlFor='includeInstallation'
                    className='text-sm text-foreground flex items-center cursor-pointer'
                  >
                    <WrenchScrewdriverIcon className='w-4 h-4 mr-1.5 text-primary/90' />
                    Añadir Servicio de Instalación (+
                    {formatCurrencyChilean(MOCK_INSTALLATION_PRICE)})
                  </label>
                </div>
              )}
              ...
              {product.productType === 'PRODUCT' &&
                ...
                product.totalStock > 0 && (
                  <p className='text-xs text-destructive mt-1'>
                    No hay suficiente stock. Solo {product.totalStock} unidades
                    disponibles.
                  </p>
                )}
