...
} from '@/components/Icons' // Para imagen placeholder y tipo
...
const ProductCard: React.FC<ProductCardProps> = ({ product }) => {
  const TypeIcon =
    product.productType === 'PRODUCT' ? CubeIcon : WrenchScrewdriverIcon
    ...
    <Link href={`/products/${product.id}`} className='group block h-full'>
      <Card className='overflow-hidden shadow-md hover:shadow-xl dark:hover:shadow-primary/30 transition-all duration-300 h-full flex flex-col border border-border hover:border-primary/50 dark:hover:border-primary/40'>
        <div className='relative w-full aspect-[4/3] bg-muted overflow-hidden'>
          ...
          {/* Product Type Badge */}
          <div
            className={`absolute top-2 right-2 text-xs font-semibold px-2.5 py-1 rounded-full shadow-sm flex items-center
            ${
          ...
          <div className='mt-auto pt-2'>
            ...
            {product.productType === 'PRODUCT' &&
              ...
              product.totalStock !== undefined && (
                <p
                  className={`text-xs mt-1 font-medium ${product.totalStock > 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-destructive'}`}
                >
