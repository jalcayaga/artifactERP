...
const InvoiceDetailModal: React.FC<InvoiceDetailModalProps> = ({ invoice, onClose, onPaymentSuccess }) => {
  ...
  const handleSavePayment = async (data: { amount: string; paymentDate: string; paymentMethod: PaymentMethod; notes?: string }) => {
    try {
      ...
      await PaymentService.createPayment({ ...data, amount: numericAmount, invoiceId: invoice.id });
  ...
  const totalPaid = payments.reduce((acc, p) => acc + parseFloat(p.amount as any), 0);
    ...
    <Dialog open={true} onOpenChange={onClose}>
      <DialogContent className="max-w-4xl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 py-4">
          ...
          <div><strong>Orden de Venta:</strong> {invoice.orderId}</div>
        </div>
          ...
          <TableBody>
            {invoice.items.map(item => (
              <TableRow key={item.id}>
                ...
                <TableCell className="text-right">{formatCurrencyChilean(item.unitPrice)}</TableCell>
                <TableCell className="text-right">{formatCurrencyChilean(item.totalPriceWithVat)}</TableCell>
              </TableRow>
            ))}
          </TableBody>
        </Table>
