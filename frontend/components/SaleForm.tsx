...
const SaleForm: React.FC<SaleFormProps> = ({ onSave, onCancel }) => {
  ...
  const handleSubmit = (e: FormEvent) => {
    ...
    const saleDetails: Sale = {
      id: saleId, clientName: clientName.trim(), saleDate, observations: observations.trim(),
      items, vatRatePercent: FIXED_VAT_RATE_PERCENT, subTotal, totalVatAmount, grandTotal,
    };
    onSave(saleDetails);
  };
  ...
  return (
    <Card className="max-w-4xl mx-auto border">
      <CardHeader>
        <CardTitle className="text-xl">Crear Nueva Venta</CardTitle>
      </CardHeader>
      <form onSubmit={handleSubmit}>
        <CardContent className="space-y-6 pt-6">
                  ...
                  <tbody className="divide-y divide-border">
                    {items.map(item => (
                      <tr key={item.id}>
                        ...
                        <td className="px-3 py-2 whitespace-nowrap text-sm text-foreground font-medium text-right">{item.totalItemPrice.toFixed(2)}</td>
          ...
          <div className="mt-6 pt-4 border-t border-border space-y-2">
            ...
            <div className="flex justify-end items-center"><span className="text-md font-medium text-muted-foreground">IVA ({FIXED_VAT_RATE_PERCENT}%):</span><span className="ml-4 text-md font-semibold text-foreground w-32 text-right">${totalVatAmount.toFixed(2)}</span></div>
            <div className="flex justify-end items-center mt-1 pt-1 border-t border-border/50"><span className="text-lg font-bold text-foreground">Total Venta:</span><span className="ml-4 text-2xl font-bold text-primary w-32 text-right">${grandTotal.toFixed(2)}</span></div>
          </div>
        ...
        <CardFooter className="flex flex-col sm:flex-row justify-end items-center gap-3 pt-6">
          ...
          <button type="submit" className="w-full sm:w-auto order-1 sm:order-2 bg-primary hover:bg-primary/90 text-primary-foreground font-medium py-2.5 px-5 rounded-lg shadow-sm hover:shadow-md transition-all duration-150 flex items-center justify-center space-x-2" aria-live="polite">
            <ShoppingCartIcon className="w-5 h-5" /> <span>Guardar Venta</span>
          </button>
        </CardFooter>
      </form>
    </Card>
