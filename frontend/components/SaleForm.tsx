...
const SaleForm: React.FC<SaleFormProps> = ({ saleData, onSave, onCancel }) => {
  ...
  const { subTotal, totalVatAmount, grandTotal } = useMemo(() => {
    const currentSubTotal = items.reduce((total, item) => total + parseFloat(item.totalPrice.toString()), 0);
    const currentTotalVatAmount = items.reduce((total, item) => total + parseFloat(item.itemVatAmount.toString()), 0);
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      toast.success(`Venta ${createdSale.id.substring(0, 8)}... creada exitosamente.`);
          ...
          <CardContent className="space-y-6 pt-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                ...
                <Input type="date" id="sale-date" value={saleDate} onChange={(e) => setSaleDate(e.target.value)} aria-describedby="saleDate-error"/>
            ...
            <div className="space-y-4 p-4 border border-border rounded-md bg-card">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-12 gap-4 items-end">
                <div className="sm:col-span-5 relative">
                  <Label htmlFor="product-search" className="block text-xs font-medium text-muted-foreground">Buscar Producto</Label>
                  <Input 
                    id="product-search" 
                    type="text" 
                    value={searchTerm}
                ...
                {selectedProductToAdd && (
                  <div className="sm:col-span-7 grid grid-cols-1 sm:grid-cols-5 gap-4">
                    ...
                    <div className="sm:col-span-3">
                      <Label htmlFor="item-price" className="block text-xs font-medium text-muted-foreground">P. Unit. Venta (sin IVA)</Label>
                      <Input 
                        type="number" 
                        id="item-price" 
                    ...
                    {selectedProductToAdd.productType === 'PRODUCT' && selectedProductLots.length > 0 && (
                      <div className="sm:col-span-5">
                        <Label htmlFor="lot-select" className="block text-xs font-medium text-muted-foreground">Seleccionar Lote (Precio Compra)</Label>
                          ...
                          <SelectContent>
                            {selectedProductLots.map(lot => (
                              <SelectItem key={lot.id} value={lot.id}>
                                Lote: {lot.lotNumber} - Cant: {lot.currentQuantity} - Compra: {formatCurrencyChilean(lot.purchasePrice)}
                              </SelectItem>
                            ))}
                          </SelectContent>
                        </Select>
                        {itemErrors.lot && <p className={errorTextClass}>{itemErrors.lot}</p>}
                      </div>
                    )}
                  ...
                  <table className="min-w-full divide-y divide-border">
                    ...
                    <tbody className="divide-y divide-border">
                      {items.map((item, index) => (
                        <tr key={index}> {/* Using index as key for now, consider unique IDs if items can be reordered */}
                          <td className="px-3 py-2 whitespace-nowrap text-sm text-foreground">{item.productId}</td> {/* Display product ID for now */}
            ...
            <div>
              ...
              <textarea id="sale-observations" value={observations} onChange={(e) => setObservations(e.target.value)} rows={3} className={inputBaseClass} placeholder="Añadir notas adicionales sobre la venta..."></textarea>
            </div>
          </CardContent>
