    ...
    >
      ...
      >
        <div className="flex items-center justify-between p-4 sm:p-5 border-b border-border">
          <h2 id="company-detail-modal-title" className="text-lg sm:text-xl font-semibold text-foreground flex items-center">
        ...
        <div className="p-4 sm:p-5 space-y-1 overflow-y-auto">
          <dl className="divide-y divide-border/50">
            ...
            {company.address && <DetailItem label="Dirección" value={`${company.address}${company.city ? `, ${company.city}` : ''}${company.state ? `, ${company.state}` : ''}${company.zip ? ` ${company.zip}` : ''}`} />}
            {company.phone && <DetailItem label="Teléfono" value={company.phone} />}
            {company.email && <DetailItem label="Email" value={company.email} />}
            <DetailItem 
              label="Roles" 
              value={ 
        ...
        <div className="pt-4 mt-4 border-t border-border/50">
            <ContactPeopleView companyId={company.id} isManagementView={false} />
          </div>
        </div>
