                ...
                <Table>
                  <TableHeader className="bg-muted/50">
                    <TableRow>
                      ...
                      <TableHead className="hidden sm:table-cell">Fecha Pedido</TableHead>
                      <TableHead className="text-right">Total</TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                    {purchaseOrders.map((purchaseOrder) => (
                      <TableRow key={purchaseOrder.id} className="hover:bg-accent transition-colors duration-150">
                        <TableCell className="font-mono text-muted-foreground">{purchaseOrder.id.substring(0, 8)}...</TableCell>
                        <TableCell>
                          <div className="font-medium text-foreground">{purchaseOrder.company?.name}</div>
                        </TableCell>
                        <TableCell className="hidden sm:table-cell text-muted-foreground">{new Date(purchaseOrder.orderDate).toLocaleDateString()}</TableCell>
                        <TableCell className="font-semibold text-primary text-right">{formatCurrencyChilean(purchaseOrder.grandTotal)}</TableCell>
                      </TableRow>
                    ))}
                  </TableBody>
                </Table>
        ...
        <div className="flex justify-between items-center mt-4">
          <div>
            <p className="text-sm text-muted-foreground">
              Página {currentPage} de {totalPages} (Total: {totalPurchaseOrders} órdenes)
            </p>
          </div>
