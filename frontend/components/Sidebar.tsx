...
const Sidebar: React.FC<SidebarProps> = ({
  ...
  const sidebarContent = (
    <>
      <div
        className={`flex items-center mb-8 px-4 ${isCollapsed ? 'justify-center' : 'justify-between'}`}
      >
        {!isCollapsed && (
          <Link
          ...
          {navItems.map((item) => (
            <li key={item.path} className='mb-1'>
              <Link
                ...
                className={`w-full flex items-center py-2.5 px-4 rounded-md transition-colors duration-150 text-sm ${
              ...
              >
                <item.icon
                  className={`w-6 h-6 ${!isCollapsed ? 'mr-3' : ''} flex-shrink-0`}
                />
                {!isCollapsed && (
  ...
  return (
    <>
      ...
      {/* Desktop Sidebar */}
      <aside
        className={`hidden md:flex flex-col bg-card text-card-foreground p-4 shadow-lg transition-all duration-300 ease-in-out
