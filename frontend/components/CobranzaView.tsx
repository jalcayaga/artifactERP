...
const CobranzaView: React.FC = () => {
  ...
  const fetchInvoices = useCallback(
    async (page: number = 1, status: InvoiceStatus | 'all' = 'all') => {
  ...
  const calculateBalance = (invoice: Invoice) => {
    const totalPaid =
      invoice.payments?.reduce((acc, p) => acc + (p.amount as any), 0) || 0
                ...
                <TableBody>
                  {invoices.map((invoice) => (
                    ...
                    >
                      ...
                      <TableCell>
                        {formatCurrencyChilean(invoice.grandTotal as any)}
                      </TableCell>
                      <TableCell>
                        {formatCurrencyChilean(calculateBalance(invoice))}
                      </TableCell>
      ...
      <div className='flex justify-between items-center mt-4'>
        <div>
          <p className='text-sm text-muted-foreground'>
            Página {currentPage} de {totalPages} (Total: {totalInvoices}{' '}
            facturas)
          </p>
        </div>
