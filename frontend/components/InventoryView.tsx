        ...
        const productsWithStock = await Promise.all(
          response.data.map(async (product: Product) => {
            if (product.productType === 'PRODUCT') {
              const lots = await ProductService.getProductLots(
                product.id,
                token
              )
    ...
    async (productData: Product, technicalSheetFile: File | null) => {
      ...
      try {
        ...
        if (technicalSheetFile && savedProduct.id) {
          await ProductService.uploadTechnicalSheet(
            savedProduct.id,
            technicalSheetFile,
            token
          )
            ...
            {products.length > 0 ? (
                      ...
                      >
                        ...
                        <td className='px-4 py-3 whitespace-nowrap text-sm font-medium'>
                          <div className='flex items-center justify-center space-x-1 sm:space-x-2'>
                            ...
                            <button
                              onClick={() =>
                                handleDeleteProductRequest(product)
                              }
              ...
              <div className='text-center py-12 px-4'>
                ...
                <p className='mt-1.5 text-sm text-muted-foreground'>
                  Empieza añadiendo tu primer producto o servicio para gestionar
                  tu inventario y catálogo.
                </p>
                <div className='mt-6'>
                  <button
                    type='button'
