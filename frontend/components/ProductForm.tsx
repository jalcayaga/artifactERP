...
import { ArchiveBoxIcon, PlusIcon, DocumentTextIcon, TrashIcon, CalculatorIcon } from '@/components/Icons';
...
const ProductForm: React.FC<ProductFormProps> = ({ productData, onSave, onCancel }) => {
  ...
  const handleSuggestPrice = () => {
    ...
    if (isNaN(cost) || cost <= 0) {
      toast.error('Por favor, ingrese un costo unitario válido para calcular el precio sugerido.');
      return;
    }
  ...
  const handleRemoveTechnicalSheet = async () => {
    if (productData && productData.id && token) {
        try {
            ...
            setErrors(prev => ({...prev, technicalSheet: 'Error al eliminar la ficha técnica.'}));
        }
    }
  };
      ...
      <form onSubmit={handleSubmit}>
        <CardContent className="space-y-5 pt-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            <div>
              <label htmlFor="prod-name" className="block text-sm font-medium text-foreground">Nombre <span className="text-red-500">*</span></label>
              <input type="text" id="prod-name" value={name} onChange={(e) => setName(e.target.value)} className={inputBaseClass} />
            </div>
            <div>
              <label htmlFor="prod-type" className="block text-sm font-medium text-foreground">Tipo <span className="text-red-500">*</span></label>
              <select id="prod-type" value={productType} onChange={(e) => setProductType(e.target.value as 'PRODUCT' | 'SERVICE')} className={selectBaseClass}>
          ...
          <div>
            ...
            <textarea id="prod-description" value={description} onChange={(e) => setDescription(e.target.value)} rows={2} className={inputBaseClass}></textarea>
          </div>

          <div>
            <label htmlFor="prod-long-description" className="block text-sm font-medium text-foreground">Descripción Larga</label>
            <textarea id="prod-long-description" value={longDescription} onChange={(e) => setLongDescription(e.target.value)} rows={4} className={inputBaseClass}></textarea>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            <div>
              <label htmlFor="prod-unit-price" className="block text-sm font-medium text-foreground">Precio Costo (sin IVA)</label>
              <input type="number" id="prod-unit-price" value={unitPrice} onChange={(e) => setUnitPrice(e.target.value)} className={inputBaseClass} placeholder="0.00" step="0.01"/>
              {errors.unitPrice && <p className={errorTextClass}>{errors.unitPrice}</p>}
            </div>
            <div className="bg-muted/50 p-3 rounded-lg">
                <label className="block text-sm font-medium text-foreground mb-2">Sugerencia de Precio</label>
                <div className="flex items-center gap-2">
                    <input type="number" value={profitMargin} onChange={(e) => setProfitMargin(e.target.value)} className={`${inputBaseClass} w-20 text-center`} placeholder="30"/>
                    <span className="text-sm text-muted-foreground">% de Utilidad</span>
                    <button type="button" onClick={handleSuggestPrice} disabled={!unitPrice || parseFloat(unitPrice) <= 0} className="px-3 py-2 bg-secondary hover:bg-secondary/80 text-secondary-foreground text-sm font-medium rounded-md shadow-sm disabled:opacity-50 disabled:cursor-not-allowed">
                        <CalculatorIcon className="w-5 h-5"/>
                    </button>
                </div>
                {averageCost !== null && <p className="text-xs text-muted-foreground mt-2">Costo Promedio Actual: {formatCurrencyChilean(averageCost)}</p>}
            </div>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            <div>
              <label htmlFor="prod-price" className="block text-sm font-medium text-foreground">Precio Venta (sin IVA) <span className="text-red-500">*</span></label>
              <input type="number" id="prod-price" value={price} onChange={(e) => setPrice(e.target.value)} className={inputBaseClass} placeholder="0.00" step="0.01"/>
            ...
            <div>
              <label htmlFor="prod-reorder-level" className="block text-sm font-medium text-foreground">Stock Mínimo</label>
              <input type="number" id="prod-reorder-level" value={reorderLevel} onChange={(e) => setReorderLevel(e.target.value)} className={inputBaseClass} placeholder="0"/>
            </div>
          </div>
          ...
          <div>
            ...
            <input type="file" id="prod-images" multiple accept="image/*" onChange={handleImageChange} className="mt-1 block w-full text-sm text-foreground file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:text-sm file:font-semibold file:bg-primary file:text-primary-foreground hover:file:bg-primary/90"/>
            <div className="mt-3 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
              {images.map((image, index) => (
                <div key={image} className="relative group">
                  <img src={image} alt={`Producto ${index + 1}`} className="w-full h-24 object-cover rounded-md" />
                  <button type="button" onClick={() => handleRemoveExistingImage(index)} className="absolute top-1 right-1 bg-destructive/80 text-destructive-foreground rounded-full p-1 text-xs opacity-0 group-hover:opacity-100 transition-opacity" title="Eliminar imagen existente">X</button>
                </div>
              ))}
              {newImages.map((image, index) => (
                <div key={image.name} className="relative group">
                  <img src={URL.createObjectURL(image)} alt={`Nueva imagen ${index + 1}`} className="w-full h-24 object-cover rounded-md" />
                  <button type="button" onClick={() => handleRemoveNewImage(index)} className="absolute top-1 right-1 bg-destructive/80 text-destructive-foreground rounded-full p-1 text-xs opacity-0 group-hover:opacity-100 transition-opacity" title="Eliminar nueva imagen">X</button>
                </div>
              ))}
            </div>
          </div>
          ...
          <div>
            ...
            {existingTechnicalSheetUrl ? (
              <div className="flex items-center gap-3 mt-2">
                ...
                <a href={`http://localhost:3001${existingTechnicalSheetUrl}`} target="_blank" rel="noopener noreferrer" className="text-sm font-medium text-primary hover:underline">Ver Ficha Actual</a>
                <button type="button" onClick={handleRemoveTechnicalSheet} className="text-destructive hover:text-destructive/80 p-1 rounded-md" title="Quitar Ficha Técnica">
                    <TrashIcon className="w-4 h-4" />
                </button>
              </div>
            ) : null}
            <input type="file" id="prod-technical-sheet" accept="application/pdf" onChange={handleTechnicalSheetChange} className="mt-2 block w-full text-sm text-foreground file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:text-sm file:font-semibold file:bg-primary file:text-primary-foreground hover:file:bg-primary/90"/>
            {technicalSheetFile && <p className="mt-2 text-sm text-muted-foreground">Nuevo archivo seleccionado: {technicalSheetFile.name}</p>}
            {errors.technicalSheet && <p className={errorTextClass}>{errors.technicalSheet}</p>}
          </div>
          
          <div className="flex items-center pt-2">
            <input id="prod-isPublished" type="checkbox" checked={isPublished} onChange={(e) => setIsPublished(e.target.checked)} className="h-4 w-4 text-primary border-border rounded focus:ring-primary"/>
            <label htmlFor="prod-isPublished" className="ml-2 block text-sm text-foreground">Publicado (visible en tienda/catálogo)</label>
          </div>
