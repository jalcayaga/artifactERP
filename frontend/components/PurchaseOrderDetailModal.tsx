...
const PurchaseOrderDetailModal: React.FC<PurchaseOrderDetailModalProps> = ({
  ...
  const getStatusBadgeClasses = (status: string) => {
    switch (status) {
      ...
      case 'Aprobada':
        return 'bg-primary/10 text-primary' // Using primary for approved
      case 'Recibida':
  ...
  return (
    ...
    >
      <div
        className='bg-card text-card-foreground rounded-lg shadow-xl w-full max-w-3xl max-h-[90vh] flex flex-col overflow-hidden'
        ...
        <div className='p-4 sm:p-5 space-y-4 overflow-y-auto'>
          <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-6 gap-y-3 mb-3'>
            ...
            <OrderInfoItem
              ...
              value={new Date(order.purchaseDate).toLocaleDateString()}
              icon={CalendarIcon}
            />
            <OrderInfoItem
              label='Estado'
              value={
                <span
                  className={`px-2.5 py-0.5 inline-flex text-xs leading-5 font-semibold rounded-full ${getStatusBadgeClasses(order.status)}`}
                >
                  {order.status}
                </span>
              }
          ...
          <div className='mt-4 pt-3 border-t border-border space-y-1.5'>
            <div className='flex justify-between items-center'>
              ...
              <span className='text-sm font-semibold text-foreground'>
                {formatCurrencyChilean(order.subTotalAmount)}
              </span>
            </div>

            <div className='flex justify-between items-center'>
              <span className='text-sm font-medium text-muted-foreground'>
                Total IVA ({order.vatRatePercent}%):
              </span>
              <span className='text-sm font-semibold text-foreground'>
                {formatCurrencyChilean(order.totalVatAmount)}
              </span>
            </div>
            ...
            <div className='flex justify-between items-center pt-1.5 border-t border-border/50'>
              ...
              <span className='text-lg font-bold text-primary'>
                {formatCurrencyChilean(order.grandTotal)}
              </span>
            </div>
          </div>
        </div>
