...
const UserOrdersPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      ...
      try {
        ...
        setError(
          'No se pudieron cargar tus pedidos. Por favor, inténtalo de nuevo.'
        )
      } finally {
        setLoading(false)
      }
  ...
  return (
    <div className='container mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-12'>
      <h1 className='text-3xl sm:text-4xl font-bold text-foreground tracking-tight text-center mb-8 md:mb-12'>
        Mis Pedidos
      </h1>
      {orders.length === 0 ? (
                  ...
                  <ul className='list-disc pl-5 space-y-1'>
                    {order.orderItems.map((item) => (
                      <li key={item.id} className='text-sm'>
                        ...
                        {formatCurrencyChilean(item.unitPriceSnapshot)}
                      </li>
                    ))}
                  </ul>
                </div>
