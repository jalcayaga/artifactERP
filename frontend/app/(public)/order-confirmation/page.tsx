    ...
    <div className='flex flex-col items-center justify-center min-h-[calc(100vh-150px)] bg-background text-foreground px-4 py-8'>
      ...
      {displayOrderId && (
        <p className='text-md text-center mb-8'>
          Número de Pedido:{' '}
          <span className='font-semibold text-primary'>{displayOrderId}</span>
        </p>
      )}
      <div className='flex flex-col sm:flex-row gap-4'>
        <Link
          href='/products'
          className='px-6 py-3 bg-primary text-primary-foreground rounded-md hover:bg-primary/90 transition-colors text-center'
        >
          Seguir Comprando
        </Link>
        {displayOrderId && (
          <Link
            href={`/admin/sales/${displayOrderId}`}
            className='px-6 py-3 border border-input bg-background rounded-md hover:bg-accent hover:text-accent-foreground transition-colors text-center'
          >
