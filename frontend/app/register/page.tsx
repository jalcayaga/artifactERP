...
const RegisterPage: React.FC = () => {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (!result.success) {
      setError(result.error || 'Fallo el registro. Por favor, intente de nuevo.');
    } else {
      // Redirigir al dashboard si el registro y el login automático fueron exitosos
  ...
  return (
    <div className="min-h-screen bg-background text-foreground flex flex-col justify-center py-12 sm:px-6 lg:px-8">
      <div className="sm:mx-auto sm:w-full sm:max-w-md text-center">
        <UserPlusIcon className="mx-auto h-12 w-auto text-primary" />
        <h2 className="mt-6 text-3xl font-extrabold text-foreground">
          Crear una cuenta en {APP_NAME}
        </h2>
        <p className="mt-2 text-sm text-muted-foreground">
          O{' '}
          <Link href="/login" className="font-medium text-primary hover:text-primary/80 dark:hover:text-primary/70">
