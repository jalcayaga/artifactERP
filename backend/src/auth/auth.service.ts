...
export class AuthService {
  ...
  async validateUser(email: string, pass: string): Promise<Omit<Prisma.User, 'password'> | null> { // Changed User to Prisma.User
    const user = await this.usersService.findOneByEmail(email);
    if (user && typeof user.password === 'string' && await bcrypt.compare(pass, user.password)) {
      // eslint-disable-next-line @typescript-eslint/no-unused-vars
      const { password, ...result } = user;
      return result as Omit<Prisma.User, 'password'>; // Changed User to Prisma.User
    }
    return null;
  }
